...
export function mapToKPIs(
  ...
  const deliveredRow = statusData.find(r => {
    return str(r['dashboard_overview.status']) === 'COMPLETED';
  });
  const cancelledRow = statusData.find(r => {
    return str(r['dashboard_overview.status']) === 'CANCELLED';
  });
  ...
  const sortedProviders = [...providerData].sort((a, b) => {
    return (
      num(b['dashboard_overview.order_items_totalRevenue']) - num(a['dashboard_overview.order_items_totalRevenue'])
    );
  });
...
export function mapToRegions(cityData: CubeResultRow[]): RegionData[] {
  return cityData
    .filter(r => {
      return str(r['dashboard_overview.user_agencies_branchName']).trim() !== '';
    })
    .map((row, i) => {
...
export function mapToTopPerformers(productData: CubeResultRow[], cityData: CubeResultRow[]): TopPerformerData[] {
  return productData
    .filter(r => {
      return str(r['dashboard_overview.order_items_packageName']).trim() !== '';
    })
    .slice(0, 7)
    .map((row, i) => {
      return {
