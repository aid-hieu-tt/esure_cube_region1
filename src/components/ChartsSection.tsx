...
export const ChartsSection: React.FC<ChartsSectionProps> = ({ timeTracking, regions, onRegionClick }) => {
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="shadow-slate-200/60 rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
          <p className="mb-1 font-bold">{label}</p>
          <p className="text-sm text-slate-600">
            Chỉ tiêu: <span className="font-semibold text-orange-500">{formatCurrency(data.target)}</span>
          </p>
          <p className="text-sm text-slate-600">
            Thực hiện: <span className="font-semibold text-blue-800">{formatCurrency(data.actual)}</span>
          </p>
          <p className="text-sm text-slate-600">
            Đạt: <span className="font-semibold">{data.completionRate}%</span>
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:gap-5">
      {/* Donut Chart */}
      <div className="shadow-slate-200/70 relative flex h-[360px] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white p-4 shadow-md lg:col-span-4">
        ...
        <div className="mb-3">
          <p className="tracking-[0.22em] text-[11px] font-semibold uppercase text-slate-500">Tiến độ</p>
          <h3 className="tracking-tight mt-1 text-base font-extrabold text-slate-900">Thời gian trong kỳ</h3>
        ...
        <div className="relative flex flex-1 items-center justify-center">
          ...
          <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center">
            <div
              className="ring-1 ring-white/15 flex flex-col items-center justify-center rounded-full bg-gradient-to-br from-slate-950 via-slate-900 to-blue-700 text-white shadow-xl"
              style={{ width: '130px', height: '130px' }}
            >
              <span className="mb-1 text-5xl font-bold leading-none">{remainingDays}</span>
      ...
      {/* Bar Chart */}
      <div className="shadow-slate-200/70 relative flex h-[360px] w-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white p-4 shadow-md lg:col-span-8">
        ...
        <div className="mb-3">
          <p className="tracking-[0.22em] text-[11px] font-semibold uppercase text-slate-500">So sánh</p>
          <h3 className="tracking-tight mt-1 text-base font-extrabold text-slate-900">Doanh số theo vùng</h3>
        ...
        <div className="w-full flex-1 overflow-x-auto overflow-y-hidden">
          <div style={{ minWidth: regions.length > 5 ? `${regions.length * 75}px` : '100%', height: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={regions} margin={{ top: 20, right: 30, left: 20, bottom: 25 }}>
                ...
                <XAxis
                  ...
                  onClick={data => {
                    return handleBarClick({ name: data.value });
                  }}
