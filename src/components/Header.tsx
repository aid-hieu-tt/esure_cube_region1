...
export const Header: React.FC<HeaderProps> = ({ onOpenSettings, onDateChange, dateValue }) => {
  return (
    <div className="shadow-slate-200/80 backdrop-blur relative overflow-hidden rounded-3xl border border-white/70 bg-white/80 p-4 shadow-lg md:p-5">
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-blue-50/70 via-white/0 to-indigo-50/60" />
      <div className="pointer-events-none absolute -right-24 -top-24 size-56 rounded-full bg-blue-400/10 blur-2xl" />
      <div className="pointer-events-none absolute -bottom-28 -left-28 size-64 rounded-full bg-indigo-400/10 blur-2xl" />

      <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <div className="tracking-[0.22em] inline-flex items-center gap-2 rounded-full border border-slate-200/70 bg-white/70 px-3 py-1 text-[11px] font-semibold uppercase text-slate-500">
            <span className="size-1.5 rounded-full bg-emerald-500" />
            Live
          </div>
          <h1 className="tracking-tight mt-3 bg-gradient-to-r from-slate-950 via-slate-900 to-blue-700 bg-clip-text text-2xl font-extrabold text-transparent md:text-3xl">
            Dashboard Chi Nhanh
          </h1>
