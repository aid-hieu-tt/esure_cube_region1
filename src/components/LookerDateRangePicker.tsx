...
export const LookerDateRangePicker: React.FC<LookerDateRangePickerProps> = ({ value = 'This month', onChange }) => {
  ...
  return (
    <div className="relative inline-block text-left" ref={containerRef}>
      <button
        ...
        className="focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:border-blue-200 hover:bg-blue-50/70"
      >
        <Calendar size={16} className="text-slate-500" />
        <span>{displayLabel}</span>
        <ChevronDown
          size={16}
          className={`ml-1 text-slate-500 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>

      {isOpen && (
        <div className="shadow-slate-200/70 ring-1 ring-black/5 absolute right-0 z-50 mt-2 flex w-80 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl lg:w-[480px] lg:flex-row">
          {/* Presets Column */}
          <div className="flex max-h-80 w-full flex-col overflow-y-auto border-b border-slate-200 py-1 lg:w-1/2 lg:border-b-0 lg:border-r">
            <div className="tracking-wider bg-slate-50 px-3 py-2 text-xs font-semibold uppercase text-slate-500">
              Mốc thời gian
            </div>
            {PRESETS.map(preset => {
              return (
          ...
          <div className="flex w-full flex-col justify-between bg-slate-50 p-4 lg:w-1/2">
            <div className="flex-1">
              <div className="flex flex-col gap-4">
                <div>
                  <label
                    htmlFor="branch-dashboard-date-from"
                    className="mb-1 block text-xs font-semibold text-slate-700"
                  >
                    Từ ngày
                  </label>
                  <input
                    id="branch-dashboard-date-from"
                    type="date"
                    value={tempCustom[0]}
                    onChange={e => {
                      return handleDateChange(0, e.target.value);
                    }}
                    className="focus:outline-none focus:ring-2 focus:ring-blue-500 w-full rounded-xl border border-blue-100 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm transition-colors focus:border-blue-500"
                  />
                </div>
                <div>
                  <label htmlFor="branch-dashboard-date-to" className="mb-1 block text-xs font-semibold text-slate-700">
                    Đến ngày
                  </label>
                  <input
                    id="branch-dashboard-date-to"
                    type="date"
                    min={tempCustom[0]}
                    value={tempCustom[1]}
                    onChange={e => {
                      return handleDateChange(1, e.target.value);
                    }}
                    className="focus:outline-none focus:ring-2 focus:ring-blue-500 w-full rounded-xl border border-blue-100 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm transition-colors focus:border-blue-500"
                  />
                </div>
